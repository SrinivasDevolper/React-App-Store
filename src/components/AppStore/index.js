...
class AppStore extends Component {
  state = {inputValue: '', listCategory: 'SOCIAL', storeappList: appsList}
  ...
  tabClick = tabId => {
    const resapptab = appsList.filter(tabset => {
      const {category} = tabset
      return category === tabId
    })
    this.setState({listCategory: tabId, storeappList: resapptab})
  }

  render() {
    const {inputValue, listCategory, storeappList} = this.state
    const resultList = storeappList.filter(eachapp => {
      const {category, appName} = eachapp
      if (listCategory === category) {
        return appName.toLowerCase().includes(inputValue.toLowerCase())
      }
      return false
    })
    return (
        ...
        <div>
          ...
          <ul className="tab-cont">
            {tabsList.map(eachTabItem => {
              const {tabId} = eachTabItem
              return (
                <TabItems
                  key={tabId}
                  eachTabItem={eachTabItem}
                  tabClick={this.tabClick}
                  isActive={tabId === listCategory}
                />
              )
            })}
          </ul>
